//Implementing an asynchronous TCP client - multithread
...
public:
    ...
    void emulateLongComputationOp(
        ...
        // the corresponding request before it completes. Because active sessions list can be accessed from
        // multiple threads, we guard it with a mutex to avoid data corruption.
    ...
    void close() {
        // Destroy work object. This allows the I/O threads to exit the event loop when there are no more pending asynchronous operations.
        m_work.reset(NULL);
...
private:
    void onRequestComplete(std::shared_ptr<Session> session) {
        // Shutting down the connection. This method may fail in case socket is not connected. We do not care about
