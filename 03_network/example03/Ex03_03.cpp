...
public:
    ...
    void emulateLongComputationOp(
        ...
        // Add new session to the list of active sessions so that we can access it if the user decides to cancel
        // the corresponding request before it completes. Because active sessions list can be accessed from multiple
...
private:
    void onRequestComplete(std::shared_ptr<Session> session) {
        // Shutting down the connection. This method may fail in case socket is not connected. We do not care about the error code if this function fails.
...
int main() {
    try {
        ...
        // Here we emulate the user抯 behavior.
...
/**
 * As it has already been mentioned in the introduction section of this chapter, the simplest asynchronous client is
 * structurally more complex than equivalent synchronous one. When we add a feature such as request canceling to the
 * asynchronous client, it becomes even more complex.
 *
 * In this recipe, we'll consider an asynchronous TCP client application supporting the asynchronous execution of the
 * requests and request canceling functionality. Here is the list of requirements the application will fulfill:
 * 1> Input from the user should be processed in a separate thread—the user interface thread. This thread should never
 * be blocked for a noticeable amount of time.
 * 2> The user should be able to issue multiple requests to different servers.
 * 3> The user should be able to issue a new request before the previously issued requests complete.
 * 4> The user should be able to cancel the previously issued requests before they complete.
 */
