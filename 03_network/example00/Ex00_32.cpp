...
/**
 * Since the timer is asynchronously executed, it is possible that the timer execution is not in a serialized process. The
 * timer might be executed in one thread while another event is executed at the same time.As we have discussed previously,
 * we can utilize the strand function to serialize the order of execution.
 */