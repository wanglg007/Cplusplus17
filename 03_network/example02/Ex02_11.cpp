    ...
    try {
        ...
        sock->async_connect(ep,
                            [sock](const boost::system::error_code &ec) {
                                // If asynchronous operation has been cancelled or an error occured during executon,
                                // ec contains corresponding error code.
                                if (ec != 0) {
...
/**
 * In order to close an allocated socket, the close() method should be called on the corresponding object of the
 * asio::ip::tcp::socket class. However, usually, there is no need to do it explicitly because the destructor of the
 * socket object closes the socket if one was not closed explicitly.
 */
