...
boost::optional<std::string> find_latest_album_of(const std::string &artiste_name) {
...
int main() {
    ...
    for (const auto &artiste : artistes) {
        boost::optional<std::string> album = find_latest_album_of(artiste);
        ...
        if (album) {
            ...
            std::cout << "No information on albums from " << artiste << '\n';
        }
    }
}