...
int main() {
    ...
    directory.insert(p1);
    directory.insert(p2);
    auto it1 = directory.lower_bound(PersonEntry{"Arindam Mukherjee", "", ""});
    auto it2 = directory.upper_bound(PersonEntry{"Arindam Mukherjee", "", ""});

    while (it1 != it2) {
        std::cout << "Found: [" << it1->name << ", " << it1->phoneNumber << ", " << it1->city << "]\n";
        ++it1;
    }
}
