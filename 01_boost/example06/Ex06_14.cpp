...
int main() {
    // check pointers
    typedef int *intptr;
    std::cout << "intptr is " << (boost::is_pointer<intptr>::value ? "" : "not ") << "pointer type\n";
    ...
    std::cout << "MyStruct is " << (boost::is_pod<MyStruct>::value ? "" : "not ") << "pod type." << '\n';
    std::cout << "std::string is " << (boost::is_pod<std::string>::value ? "" : "not ") << "pod type." << '\n';
}
