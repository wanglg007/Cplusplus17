 ...
 *
 * 处理该情况的常规方法：使线程函数的功能齐全，并将数据复制到线程中，而非复制到共享数据中。如果使用一个可调用的对象作为线程函数，该对象就会复
 * 制到线程中，而后原始对象会立即销毁。但对于对象中包含的指针和引用还需谨慎。使用一个能访问局部变量的函数去创建线程是一个糟糕的主意(除非十分确
 * 定线程会在函数完成前结束)。此外，可以通过加入的方式来确保线程在函数完成前结束。
 *
 */